...
import { Trophy, Users, IndianRupee, Trash2, CheckCircle, Gamepad2 } from "lucide-react";
...
const handleRegister = async (tournamentId) => {
  try {
    await API.post("/tournaments/register", { tournamentId }, {
...
const filteredTournaments = tournaments.filter((tournament) => {
  ...
  const matchesEntryFee =
    !maxEntryFee || tournament.entryFee <= Number(maxEntryFee);

  const matchesPrizePool =
    !minPrizePool || tournament.prizePool >= Number(minPrizePool);
    ...
    >
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        <div
          ...
          style={{
            background: "linear-gradient(135deg,rgba(124,58,237,0.2) 0%,rgba(10,22,40,0.8) 60%,rgba(6,182,212,0.08) 100%)",
            border: "1px solid rgba(168,85,247,0.2)",
          }}
        >
          <div className="absolute -top-8 -right-8 w-48 h-48 rounded-full pointer-events-none"
            style={{ background: "radial-gradient(circle,rgba(168,85,247,0.15) 0%,transparent 70%)", filter: "blur(20px)" }} />
          <span className="absolute top-0 left-0 w-5 h-5 border-t-2 border-l-2 rounded-tl-2xl" style={{ borderColor: "rgba(168,85,247,0.5)" }} />
          <span className="absolute bottom-0 right-0 w-5 h-5 border-b-2 border-r-2 rounded-br-2xl" style={{ borderColor: "rgba(168,85,247,0.5)" }} />

          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="p-3.5 rounded-xl"
                style={{ background: "linear-gradient(135deg,rgba(168,85,247,0.3),rgba(124,58,237,0.15))", border: "1px solid rgba(168,85,247,0.4)", color: "#a855f7" }}>
                <Trophy size={28} strokeWidth={1.5} />
              </div>
              <div>
                <p className="text-[11px] font-semibold tracking-[2px] uppercase mb-1" style={{ color: "rgba(168,85,247,0.7)" }}>
                  {isAdmin ? "Admin Panel" : "Arena"}
                </p>
                <h1 className="text-[24px] font-black tracking-wide bg-clip-text text-transparent"
                  style={{ fontFamily: "'Orbitron', sans-serif", backgroundImage: "linear-gradient(135deg,#f0f4ff,#a855f7)" }}>
                  TOURNAMENTS
                </h1>
                <p className="text-[13px] mt-1" style={{ color: "rgba(148,163,184,0.6)" }}>
                  {isAdmin ? "Manage all esports tournaments" : "Browse and register for competitions"}
                </p>
              </div>
            </div>
            <div
  ...
  />
        ...
        {tournaments.length === 0 ? (
          <div className="rounded-2xl p-14 text-center"
            style={{ background: "rgba(6,12,20,0.8)", border: "1px solid rgba(168,85,247,0.12)" }}>
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-5"
              style={{ background: "rgba(168,85,247,0.08)", border: "1px solid rgba(168,85,247,0.2)", color: "rgba(168,85,247,0.5)" }}>
              <Trophy size={32} strokeWidth={1.5} />
            </div>
            <h2 className="text-[18px] font-bold mb-2" style={{ color: "#f0f4ff", fontFamily: "'Orbitron', sans-serif" }}>
              NO TOURNAMENTS YET
            </h2>
            <p className="text-[14px]" style={{ color: "rgba(148,163,184,0.5)" }}>
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg"
                      ...
                      <span className="text-[11px] font-semibold tracking-wide" style={{ color: "rgba(168,85,247,0.8)" }}>
                        {t.game}
                      </span>
                    </div>
                    <span
                  ...
                  {/* Title */}
                  <h2 className="text-[17px] font-black tracking-wide mb-4" style={{ color: "#f0f4ff", fontFamily: "'Orbitron', sans-serif", lineHeight: 1.3 }}>
                    {t.name.toUpperCase()}
                  </h2>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    {[
                      { icon: IndianRupee, label: "Prize Pool", value: `₹${t.prizePool.toLocaleString()}`, color: "#4ade80" },
                      { icon: IndianRupee, label: "Entry Fee",  value: `₹${t.entryFee.toLocaleString()}`,  color: "#facc15" },
                    ].map(({ label, value, color }) => (
                      <div key={label} className="rounded-xl px-3 py-3"
                        style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
                        <p className="text-[10px] tracking-wide mb-1" style={{ color: "rgba(148,163,184,0.5)" }}>{label}</p>
                  ...
                  <div className="mb-5">
                    <div className="flex items-center justify-between mb-1.5">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[11px]" style={{ color: "rgba(148,163,184,0.5)" }}>Teams</span>
                      </div>
                  ...
                  <div className="mt-auto">
                    ...
                    {isAdmin && (
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() => navigate(`/edit-tournament/${t._id}`)}
                          className="flex-1 py-3 rounded-xl text-[12px] font-bold tracking-[1px] uppercase flex items-center justify-center gap-2 transition-all duration-200"
                          style={{
                          ...
                          onMouseOver={(e) => {
                            ...
                            e.currentTarget.style.boxShadow = "0 0 16px rgba(249,115,22,0.3)";
                          }}
                          onMouseOut={(e) => {
